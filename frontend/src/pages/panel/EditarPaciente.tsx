...
export default function EditarPaciente() {
  ...
  useEffect(() => {
    const cargar = async () => {
      try {
        ...
        const prio = (p.prioridad ?? "normal") as "normal"|"alta"|"urgente";
        setPrioridad(prio);
        reset({
          nombre:                  p.nombre,
          apellido:                p.apellido,
          ...
          edad:                    p.edad,
          sexo:                    p.sexo as "masculino"|"femenino"|"otro"|undefined,
          email:                   p.email ?? "",
          celular:                 p.celular ?? "",
          domicilio:               p.domicilio ?? "",
          localidad:               p.localidad ?? "",
          derivado_por:            p.derivado_por ?? "",
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      await pacienteService.actualizar(Number(id), payload);
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <form onSubmit={handleSubmit(onSubmit as any)} className="space-y-5">
        ...
        <Seccion icon={AlertTriangle} titulo="Prioridad clínica">
          ...
          {prioridad !== "normal" && (
            <Campo label={`Motivo de prioridad${prioridad === "urgente" ? " *" : ""}`}>
        ...
        <div className="flex items-center justify-end gap-3">
          <button type="button" onClick={() => navigate(`/panel/pacientes/${id}`)}
